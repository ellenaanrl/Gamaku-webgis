    ...
    <div class="flex-grow">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="/map" class="text-[#fdcb2c] px-3 py-2 text-sm font-medium">Peta</a>
        ...
        <main class="max-w-7xl mx-auto py-4 sm:px-6 lg:px-8">
            <div class="px-4 py-6 sm:px-0">
                <h2 class="text-2xl font-bold mb-4">Peta Interaktif Jalan dan Bangunan </h2>
                <div class="mb-4 flex flex-col sm:flex-row gap-4">
                    <input type="text" id="globalSearchInput" placeholder="Cari nama, unit, jenis, atau nama jalan/polygon..." class="w-full sm:w-64 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#083d62] focus:border-transparent">
                    <select id="categoryFilter" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#083d62] focus:border-transparent">
                        <option value="all">Semua</option>
                        <option value="point">Titik</option>
                        <option value="polygon">Polygon</option>
                        <option value="jalan">Jalan</option>
                    </select>
                    <select id="subCategoryFilter" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#083d62] focus:border-transparent">
        ...
        function unifiedFilterAndDisplay() {
            ...
            if (category === 'point') {
                ...
                L.geoJSON({
                    ...
                    onEachFeature: function(feature, layer) {
                        const nama = feature.properties?.nama || feature.properties?.name || 'Tanpa Nama';
    ...
    `;
        ...
        fetch('/polygon')
            ...
            .then(data => {
                ...
                let colorIdx = 0;
                const palette = [
                    '#007BFF', '#28a745', '#fdcb2c', '#dc3545', '#6f42c1', '#fd7e14', '#17a2b8', '#4c8a65', '#e83e8c', '#0d6efd', '#ffc107', '#198754', '#a78fbf', '#0dcaf0', '#adb5bd', '#ff6f61', '#8bc34a', '#ff9800', '#635f30', '#9c27b0'
                ];
                ...
                (allPolygonFeatures).forEach(f => {
                    ...
                    if (!colorMap[nama]) {
                        colorMap[nama] = palette[colorIdx % palette.length];
                        colorIdx++;
                    }
                });
                if (document.getElementById('categoryFilter').value === 'polygon') updateSubCategoryOptions();
                // Add polygon color legend dynamically
